...
export const ResultView: React.FC<ResultViewProps> = ({
  ...
  const currentChapter =
    project.chapters[project.chapterIndex ?? 0] || project.chapters[0];

  return (
    <main className="max-w-7xl mx-auto px-8 py-16">
      {/* Title and actions share a baseline, the way the Library header does —
          `items-end` rather than `items-start`, so the buttons sit on the line
          the display type actually rests on instead of floating up against its
          leading. */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        <div className="shrink-0 self-start md:self-auto flex items-center gap-3 flex-wrap">
          <button
            onClick={onReturnLibrary}
            className="border border-[#2c2c2c] hover:bg-[#2c2c2c]/10 text-[#2c2c2c] transition-all rounded-[2px] px-6 py-4 text-xs font-semibold tracking-[0.2em] uppercase cursor-pointer"
          >
            Return to Library
          </button>
          <button
            onClick={onBackToPipeline}
            className="bg-[#2c2c2c] hover:bg-[#292622] text-[#d8cbb8] hover:text-[#d49653] transition-all rounded-[2px] px-6 py-4 text-xs font-semibold tracking-[0.2em] uppercase cursor-pointer shadow-sm"
          >
            ← Back to Pipeline Studio
          </button>
          <button
            onClick={() => setIsPresentationOpen(true)}
            className="bg-[#d49653] hover:bg-[#c38542] text-[#292622] font-semibold rounded-[2px] px-6 py-4 text-xs tracking-[0.2em] uppercase transition-all cursor-pointer shadow-sm"
          >
            ✦ Present Slide Deck
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.55fr_0.45fr] gap-14 items-start">
        ...
        <div className="border border-[#b6ab9c] bg-[#d8cbb8] p-8 shadow-md">
          <div className="aspect-[16/10] plate-canvas border border-[#b6ab9c] flex flex-col justify-between p-8 relative overflow-hidden">
            ...
            <div className="z-10 bg-[#d8cbb8]/90 backdrop-blur-sm -mx-8 -mb-8 p-5 border-t border-[#b6ab9c] flex items-center justify-between">
              <div>
                <div className="text-[10px] tracking-widest font-semibold uppercase text-[#978e81]">
                  FEATURING CAST
                </div>
              ...
              <div className="text-right">
                ...
                <div className="text-xs font-semibold uppercase text-[#d49653]">
                  {project.style || "Ink & Wash"}
                </div>
              </div>
            </div>
        ...
        <aside className="flex flex-col gap-6">
          <div className="border-t border-[#2c2c2c] pt-5">
            ...
            <div className="font-serif text-2xl text-[#2c2c2c] leading-snug">
              {project.style || "Ink & Wash"}
            </div>
          </div>
          ...
          <div className="border-t border-[#b6ab9c] pt-5">
            ...
            <div className="flex flex-col gap-3">
              {project.characters.map((c) => (
                <div
                  key={c.id}
                  className="flex items-center gap-3 bg-[#bfb4a3]/20 p-3 border border-[#b6ab9c]/60"
                >
                  {c.portraitUrl ? (
                    <img
                      src={c.portraitUrl}
                      alt={c.name}
                      className="w-10 h-10 object-cover rounded-[1px] border border-[#b6ab9c]"
                    />
                  ) : (
                  ...
                  <div>
                    <div className="font-serif text-lg leading-tight text-[#2c2c2c]">
                      {c.name}
                    </div>
                    <div className="text-[10px] text-[#615b53]">
                      {c.description || "Adult cast member"}
                    </div>
                  </div>
                </div>
              ))}
