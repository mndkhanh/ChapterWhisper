...
Manages manuscript projects, step progression state machine, conversational Gemini chaining, and asset storage. Implements Milestone 2 & 3 of [[Progress]].

## API Surface

| Endpoint | Method | Purpose |
| --- | --- | --- |
| projects | GET | List all projects belonging to the authenticated user |
| projects | POST | Create a new project, ingest book text once (Step 00 anchor) |
| projects/:id | GET | Retrieve full project details & step statuses |
| projects/:id/steps/:stepIndex/run | POST | Execute step 0..4 against Gemini with optional custom style override |
| projects/:id/portraits/:charId | GET | Stream character portrait PNG image |
| projects/:id/illustrations/:chId | GET | Stream chapter illustration plate PNG image |
...
3. **Step 02 ‐ Characters**: Extracts primary adult characters (strictly enforced to **max 2 adults**). Unlocks Step 3.
...
5. **Step 04 — Chapter Scene**: Formulates detailed chapter illustration prompt (strictly enforced to **max 1 chapter**). Unlocks Step 5.
6. **Step 05 ‐ Illustration Plate**: Synthesizes the final composition plate referencing previous character portraits and saves to data/storage/{projectId}/illustrations/. Project complete.
...
- **Completed Steps Are Final**: A step whose status is `done` returns **409 Conflict** and is never re-run. Regeneration is deliberately not offered — a finished plate is not spent again, and step 01 in particular could not honestly regenerate (it only calls Gemini when the style or its interaction id is missing, so a re-run silently did nothing while reporting success).
